            ...
            <div class="card-body">
                <div class="row g-3 mb-4">
                    ...
                    {{-- <div class="col-md-3">
                        ...
                        <select name="Supplier" id="supplier"
                            ...
                            <option value="{{ $supplier->id }}" {{ old('Supplier', $bm->Supplier ??
                    ...
                    <div class="col-md-3">
                        ...
                        <input type="text" name="Keterangan" id="keterangan"
                            ...
                            value="{{ old('Keterangan', $bm->Keterangan ?? '') }}" placeholder="Keterangan">
                        @error('Keterangan')
                            <div class="text-danger mt-1">{{ $message }}</div>
                        @enderror
                    </div>
                            ...
                            <div id="drop-area-invoice" class="border border-2 border-dashed rounded p-3 text-center"
                                ...
                                <div id="invoice-preview" class="mb-2">
                                    @if(!empty($bm->Invoice))
                                        ...
                                        @endphp
                                        @if($isImage)
                                            <img src="{{ asset('storage/Invoice/' . $bm->Invoice) }}" alt="Invoice"
                                        ...
                                        @else
                                            <div>
                                                ...
                                                @elseif(in_array($ext, ['zip', 'rar']))
                                                    <i class="fa fa-file-archive text-secondary fa-2x"></i>
                                                @elseif($ext == 'txt')
                                                    <i class="fa fa-file-alt text-muted fa-2x"></i>
                                                @else
                                                    <i class="fa fa-file fa-2x"></i>
                                                @endif
                                                <div class="mt-2">
                                                    <a href="{{ asset('storage/' . $bm->Invoice) }}"
                                                        target="_blank">{{ basename($bm->Invoice) }}</a>
                                                </div>
                                            </div>
                                        @endif
                                    @endif
                                </div>
                                <span id="drop-text-invoice" @if(!empty($bm->Invoice)) style="display:none;" @endif>
                                    Seret & lepas file di sini, atau klik untuk memilih file (gambar/dokumen)
                                </span>
                            </div>
                        </div>
                        @error('Invoice')
                ...
                <div class="table-responsive">
                    <table class="table align-middle" id="table-barang-masuk">
                        ...
                        <tbody>
                            @if(isset($bm->DetailBarangMasuk) && count($bm->DetailBarangMasuk))
                                @foreach($bm->DetailBarangMasuk as $i => $detail)
                                    <tr>
                                        <td>
                                            <select name="idProduk[]" class="form-control select2 produk-select" required>
                                                <option value="">Pilih Produk</option>
                                                @foreach($produk ?? [] as $p)
                                                    <option value="{{ $p->id }}" {{ old('idProduk.' . $i, $detail->idProduk) == $p->id ? 'selected' : '' }}>{{ $p->Nama }}</option>
                                                @endforeach
                                            </select>
                                        </td>
                                        <td>
                                            <input type="number" name="Qty[]" class="form-control qty-input" min="1"
                                                value="{{ old('Qty.' . $i, $detail->Qty) }}" required>
                                        </td>
                                        <td>
                                            <input type="text" name="HargaModal[]" class="form-control harga-input" min="0"
                                                value="{{ old('HargaModal.' . $i, number_format($detail->HargaModal, 0, ',', '.')) }}"
                                                required>
                                        </td>
                                        <td>
                                            <input type="text" class="form-control subtotal-input" name="Subtotal[]"
                                                value="{{ old('Subtotal.' . $i, number_format($detail->Qty * $detail->HargaModal, 0, ',', '.')) }}"
                                                readonly>
                                        </td>
                                        <td class="text-center">
                                            <button type="button" class="btn btn-danger btn-sm btn-remove-row" @if($loop->first)
                                            disabled @endif>
